...
import uk.khall.image.shape.AreaPixels;
 ...
 */
public class AverageSlowOutlineFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage substrateImage, BufferedImage finalImage) {
        ...
        if (substrateImage != null) {
            ...
            for (Color color : colors) {
                AreaPixels areaPixels = cs.getAreaPixels(color,1);
                Area area = areaPixels.getArea();
                ArrayList<Point> pixelist = areaPixels.getPixelList();
//                if (!area.isSingular()) {
//                    try {
...
//                    }
//                } else {
                double areasize = pixelist.size();
                AreaColor areaColor = new AreaColor(area, color, totalSize - areasize, pixelist);
                sortedAreas.add(areaColor);
                total += areasize;
